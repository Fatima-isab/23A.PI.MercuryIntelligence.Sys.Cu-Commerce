...
BEGIN
    ...
    SELECT CONCAT(' Productos: ', IFNULL(GROUP_CONCAT(CONCAT(productos.Nombre) SEPARATOR ', '), ''))
    INTO mensajeProductos
    FROM folios
    JOIN productos ON folios.IdFolios = productos.IdProductos
    WHERE folios.IdFolios = idFolios;
    
    -- Consulta nombre completo del vendedor y comprador
    SELECT CONCAT('. Nombre comprador: ', IFNULL(GROUP_CONCAT(CONCAT(p.Nombres) SEPARATOR ', '), ''))
    ...
    SELECT CONCAT('. Nombre vendedor: ', IFNULL(GROUP_CONCAT(CONCAT(p.Nombres) SEPARATOR ', '), ''))
